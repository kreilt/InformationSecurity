...
func main() {
	...
	fmt.Println("\nРАСШИФРОВЫВАЕМ")
	handlerDecoder(encrypted, m1, m2) //расшифровываем декодером, который пытается определить ветку
	handlerCoder(encrypted, m2, m1)   //расшифровываем тем же кодером, поменяв алфавиты местами
}
...
func decoder(bigram []rune, m1, m2 [][]rune) []rune {
	var i1, j1 int = -1, -1 //объясвляем переменные для памяти позиций
	var i2, j2 int = -1, -1
	var i1alter, j1alter int = -1, -1 //переменные для памяти позиций при обратном чтении алфавитов
	var i2alter, j2alter int = -1, -1
	out := make([]rune, 2)
	for i := 0; i < rows; i++ {
		for j := 0; j < cols; j++ {
			...
			if bigram[0] == m1[i][j] { //тот же 1 символ ищем в 1 алфавите - обратное чтение
				i1alter = i //запоминаем позиции
				j1alter = j
			}
			if bigram[1] == m2[i][j] { //тот же 2 символ ищем во 2 алфавите - обратное чтение
				i2alter = i //запоминаем позиции
				j2alter = j
			}
		}
	}
